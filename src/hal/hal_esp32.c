...
void init_io(void)
{
    ...
    if (pin_rst != LMIC_UNUSED_PIN && pin_rst != VIRTUAL_EXTENDER_PIN)
    {
    ...
    ESP_LOGI(TAG, "LORAWAN IO initialized");
}
...
void init_spi(void)
{
    ...
    ESP_ERROR_CHECK(ret);
    ESP_LOGI(TAG, "LoRaWAN SPI device added OK");
}
...
void hal_spi_write(u1_t cmd, const u1_t *buf, size_t len)
{
    ...
    esp_err_t err = spi_device_polling_transmit_synchronized(spi_handle, &spi_transaction);
    ESP_ERROR_CHECK(err);
}
